...
__version__ = "0.9.56 $Revision: 2616 $" 
#    $Date: 2011-04-22 12:23:07 +0200 (Fri, 22 Apr 2011) $
...
# changes:
# 11/04/22: bug-fix fixed_variables in combination with scaling
...
def fmin(func, x0, sigma0=None, args=()
    ...
    try: # pass on KeyboardInterrupt
        ...
        incpopsize = Options({'incpopsize':incpopsize}).eval('incpopsize')  # evaluated value
...
class CMAEvolutionStrategy(OOOptimizer):
    ...
    #       that they can be treated "absolut" rather than "relative")
        ...
        for s in solutions:
            ...
            if type(pop[-1]) is Solution and pop[-1].isgeno:
                ...
                #  return Solution(gp.geno(self), copy=False, geno=True) 
                pop[-1] = self.gp.geno(pop[-1]) # TODO qqqqqqqq: should be avoided, if pop[-1] is Solution type
        ...
        if check_points:  # useful in case of injected solutions and/or adaptive encoding
            for k in xrange(sp.popsize):  # TODO: mu would be enough without rank-lambda update  
                pop[k] = self.clampToIsoVolume(pop[k] - mold) + mold
        ...
        # check Delta m (this is not default, but should probably become at some point)
        if 11 < 3 and check_points: 
            cmean = self.sp.cmean * self.clampToIsoVolume(
                (self.sp.mueff / self.sp.cmean) * (self.mean - mold))
                                  # how about upper_length = 1.1 * sqrt(N)
        else:
            cmean = self.sp.cmean
    ...
    def clampToIsoVolume(self, delta_x, factor=1, upper_length=None):
        ...
        :Arguments:
            ...
            upper_length = (N - 1)**0.5 + 2 by default
